...
def gram_rbf(x: np.ndarray, threshold: float = 1.0) -> np.ndarray:
    ...
    sq_distances = (
        -2.0 * dot_products
        + np.expand_dims(sq_norms, axis=1)
        + np.expand_dims(sq_norms, axis=0)
    )
    sq_median_distance = np.median(sq_distances)
    return np.exp(-sq_distances / (2.0 * threshold ** 2 * sq_median_distance))
...
def _debiased_dot_product_similarity_helper(
    xty, sum_squared_rows_x, sum_squared_rows_y, squared_norm_x, squared_norm_y, n: int
):
...
def feature_space_linear_cka(
    features_x: np.ndarray, features_y: np.ndarray, debiased: bool = False
) -> np.ndarray:
